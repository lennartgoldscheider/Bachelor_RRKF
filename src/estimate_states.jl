...
function estimate_states(
    ...
    for (t, y) in zip(times[2:end], observations[2:end])
        ...
        (μ⁻, Π_sqrt), (G, b, backwards_S_sqrt) = if smooth
            ...
            #bzw. statt zweiten A: ssm.dynamics.drift_matrix
            #A ist \Phi, da es die Transitionmatrix ist und aber auch die aus der ODE
            prediction_rrkf(A, current_m, current_P_sqrt, ssm.dynamics.drift_matrix, ssm.dynamics.diffusion_matrix, current_dt, 1, Q_sqrt),
